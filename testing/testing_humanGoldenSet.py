...
for genome_name, genome, mitochondrial_chromosome in [("hg38", hg38_genome, "chrM"), ("hg19", hg19_genome, "chrMT")]:
    ...
    tmpdir_cluster = "/gpfs/scratch/bsc40/bsc40395"
    for f in os.listdir(tmpdir_cluster): fun.delete_file_or_folder("%s/%s"%(tmpdir_cluster, f))

    # add options depending on the machine
    if run_in_cluster is True: cmd += " --job_array_mode job_array --tmpdir %s"%tmpdir_cluster
    ...
    if len(all_lines_jobfile)==1 and run_in_cluster is True:
        ...
        if cluster_name=="MN4": 
            ...
            fun.run_jobarray_file_MN4_greasy(jobs_filename, name, time=time, queue=queue, threads_per_job=threads, nodes=nodes, constraint="highmem")
